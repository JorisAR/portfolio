...
The surface nets algorithm works as follows. First, we sample SDF values in a voxel grid. Secondly, we calculate intersections of the SDF surface with each grid line. We can do this by checking if the sign between two grid points A and B is different, and if so, we calculate the point of intersection (PoI) using: 
